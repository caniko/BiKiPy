...
def test_video_metadata_add():
    video_test_complete = video_test_a + video_test_b
    assert video_test_complete

    # video_test has every video metadata defined
    assert all(np.any(value) for value in video_test_complete.unfiltered_video_metadata.values())
