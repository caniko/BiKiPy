...
from .event_data_getters import *
...
def get_high_velocity_locations(df, body_part, max_vel, fps=30):
    ...
    while index < hv_index_interval.size:
        ...
        before_ev = get_before_event(event_start_index, event_end_index,
                                     position, hv_index, fps)
        following_ev = get_following_event(event_start_index, event_end_index,
                                           position, hv_index_interval.size,
                                           hv_index, fps)
...
def get_flicks_locations(df, body_part, max_vel):
    velocity = get_velocity(df, body_part)
