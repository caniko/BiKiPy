...
def get_before_event(event_start_index, event_end_index, position, hv_index, fps):
    ...
    if last_event_interval > fps:
        ...
        before_event = np.nanmean(
            position[event_start - last_event_interval + 1 : event_start + 1]
        )
    else:
        print(
...
def get_following_event(
    event_start_index, event_end_index, position, max_index, hv_index, fps
):
    event_start = hv_index[event_start_index]
    event_end = hv_index[event_end_index]
    ...
    if following_event_interval > fps:
        following_event_pos_mean = np.nanmean(position[event_end + 1 : event_end + fps])
    elif 10 < following_event_interval < fps:
        following_event_pos_mean = np.nanmean(
            position[event_end + 1 : event_end + following_event_interval - 1]
        )
    else:
        print(
