import numpy as np


def _unit_vector(vector):
    """ Returns the unit vector of the vector. """
    return vector / np.linalg.norm(vector)


def inner_angle(
    vector_a: np.array, vector_b: np.array, clockwise: bool = True
) -> np.array:
    """ Returns the inner_angle in radians between two vectors

        :param vector_a: numpy array containing the location of per time
        :param vector_b: vector pair of vector_a
        :param clockwise: converts the inner angle to clockwise angle if True

        >>> inner_angle((1, 0, 0), (0, 1, 0))
        1.5707963267948966
        >>> inner_angle((1, 0, 0), (1, 0, 0))
        0.0
        >>> inner_angle((1, 0, 0), (-1, 0, 0))
        3.141592653589793
    """
    vector_1_u = np.apply_along_axis(_unit_vector, 1, vector_a)
    vector_2_u = np.apply_along_axis(_unit_vector, 1, vector_b)

    result_angle = np.arccos(
        np.clip(np.einsum("ij,ij->i", vector_1_u, vector_2_u), -1.0, 1.0)
    )

    if clockwise:
        # Compute determinants and store them in a vertical stack
        determinants = np.zeros((len(vector_1_u), 1))
        for i in range(len(vector_1_u)):
            determinants[i] = np.linalg.det(np.vstack((vector_1_u[i], vector_2_u[i])))

        clockwise_angle = np.where(
            determinants.T >= 0, result_angle, 2 * np.pi - result_angle
        )[0]
    else:
        return result_angle

    return clockwise_angle


def angle_over_time(kinpy_df, point_a, point_b, point_c):
    df = kinpy_df.df

    position = {}
    for point in (point_a, point_b, point_c):
        position[point] = df.loc[:, [(point, "x"), (point, "y")]].values

    ab_vec = position[point_a] - position[point_b]
    bc_vec = position[point_b] - position[point_c]

    angles = inner_angle(ab_vec, bc_vec)
    accuracy_score = (
        df.loc[:, [(point_a, "likelihood")]].values
        + df.loc[:, [(point_b, "likelihood")]].values
        + df.loc[:, [(point_c, "likelihood")]].values
    ) / 3

    return {"Angles": angles, "Accuracy Score": accuracy_score.T[0]}
