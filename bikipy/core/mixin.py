...
class AbstractFeatureCollectorMixin(BikipyHashable, ABC):
    ...
    @cached_property
    def self_cache_path(self) -> Optional[FilePath]:
    ...
    def load_self_from_cache(self) -> Optional["AbstractFeatureCollectorMixin"]:
        if self.self_cache_path and self.self_cache_path.exists():
            return compress_pickle.load(self.self_cache_path)

    @property
    def analysis_series(self) -> pd.Series:
        if instance := self.load_self_from_cache():
            return instance.compute_analysis_series()
        ...
        if self.self_cache_path:
            compress_pickle.dump(self, self.self_cache_path)
