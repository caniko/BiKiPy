...
def parallel_point_inside_polygon(
    points: NDArrayFp64, polygon: NDArrayFp64, merge_ends: bool = True
...
def is_inside_sm(point: NDArrayFp64, polygon: NDArrayFp64):
    """
...
if runtime_settings.disable_numba:
    ...
    def is_inside_sm_parallel(
        points: np.ndarray[float, np.dtype[np.float64]], polygon: np.ndarray[float, np.dtype[np.float64]]
    ) -> np.ndarray[bool, bool]:
        ln = len(points)
