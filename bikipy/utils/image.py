...
def read_image(image: FilePath | NDArrayUint8, imread_flagg: Optional[list] = None) -> np.ndarray[int, np.uint8]:
...
@validate_arguments
@lru_cache
def read_image_from_path(image_path: FilePath, imread_flagg: Optional[list] = None) -> np.ndarray[int, np.uint8]:
