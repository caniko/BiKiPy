...
from pydantic_numpy.typing import Np2DArrayFp64, NpNDArrayInt16
...
def read_makesense_rectangle(
    ...
    if "label_name" in result.index:
        result.drop("label_name", axis=0, inplace=True)
    if len(result) == 3:
...
def get_all_lines_from_makesense_line_df(line_df: pd.DataFrame) -> Np2DArrayFp64:
...
def get_point_from_makesense_row(row: pd.Series) -> Np2DArrayFp64:
    return row.values[1:3]


# Use get_only_point_from_makesense instead!
# def read_first_makesense_point(*args, **kwargs) -> Np2DArrayFp64:
...
def get_only_point_from_makesense(data_path: FilePath) -> Np2DArrayFp64:
