...
class PluginChangeReference(BasePluginFile):
    ...
    human_readable_index = "ChangeReferenceImageName"
    ...
    def trialwise_and_metadata(self, trial_id: str | PositiveInt) -> Perimeter:
        trial_id_image_name = self.ingress.metadata.loc[trial_id, self.human_readable_index]
        reference_data = self.image_name_to_re_referencing_point[trial_id_image_name]

        return self.ingress.metadata_and_global_perimeter_and_perimeter_set[
            self.original_label
        ].globally_defined.change_reference(
    ...
    def globally_defined(self) -> Perimeter:
        ...
        result = self.ingress.metadata_and_global_perimeter_and_perimeter_set[
            self.original_label
        ].globally_defined.change_reference(
            get_point_from_makesense_row(reference_data), makesense_image_name=reference_data["image_name"]
        )
