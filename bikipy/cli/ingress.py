...
from bikipy.reader.ingress.sequence import sequence_generate_configuration
...
@click.option("-s", "--data_file_suffix", "kinematic_data_file_extension")
@click.option("-a", "animals_have_plural_trial_sets", is_flag=True)
def sequence(
    experiment_name: str,
    root_directory: Optional[DirectoryPath],
