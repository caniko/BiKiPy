...
analysis_data = {
    0: RESULTS_ANALYSIS_DIR
    / "Y-maze_07.06.2020 (1A) & Y-maze (after)_26.08.2020 (2A).parquet",
    1: RESULTS_ANALYSIS_DIR
    / "Y-maze2_31.08.2020 (1B) & Y-maze2 (after)_25.11.2020 (2B).parquet",
}


for i in range(2):
    after_df, before_df = list(
        pd.read_parquet(analysis_data[i]).groupby(axis=1, level=0)
    )
    before_df = before_df[1].droplevel(axis=1, level=0)
    after_df = after_df[1].droplevel(axis=1, level=0)
    analysis_df = before_df - after_df

    metadata_df = pd.read_excel("y-maze_metadata.xlsx", sheet_name=i).set_index(
        "Animal ID"
    )
