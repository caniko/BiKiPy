...
from bikipy.border.parallelogram import ParallelogramBorder
...
for subdir in os.listdir(str(DATA_DIR)):
    ...
    for file_path in glob(os.path.join(str(DATA_DIR / subdir), "**.h5")):
        ...
        data_dict[
            (subdir, exp_id)] = ParallelogramBorder.detect_sequential_border_presence(
            dlc_obj["mid-left_ear-right_ear"], borders
        )
